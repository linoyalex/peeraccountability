...
- Operations needing atomicity (vote resolution, stale-proof clearing) go through a SQL function with row locking (`FOR UPDATE`) — never an ad hoc client-side update. Default functions to `SECURITY INVOKER`; use `SECURITY DEFINER` only when an intentional RLS bypass is required, and then set `search_path = ''`, schema-qualify every object, check `auth.uid()` explicitly, and test both the allowed and denied case. **`REVOKE ... FROM PUBLIC` alone is not enough** — confirmed live on this project: Supabase auto-grants `EXECUTE` on every new `public`-schema function to `anon`, `authenticated`, and `service_role` via a project-level default ACL, and revoking from the `PUBLIC` pseudo-role doesn't touch those separately-held grants. Revoke from `anon`/`authenticated`/`service_role` explicitly by name for anything that shouldn't be client-callable, then verify with `has_function_privilege('<role>', '<function>', 'execute')` against the real database — don't trust the SQL text alone.
