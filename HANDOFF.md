...
- **Vercel deployment status, by era — don't read these as one thing:** every deployment from the Part 6 work failed (8 in a row, preview and production), exposed through GitHub only as a generic deployment-failed status. PR #3's preview **succeeded** (`● Ready`, `Build Completed in /vercel/output`) once the framework pin landed, which is the hosted-builder proof that the fix is real rather than local-only. Production is still red and stays that way until PR #3 merges, because `main` has no `vercel.json` yet — re-check it immediately after merging.
...
1. Merge PR #3 (`fix/vercel-framework-preset`): the `vercel.json` framework pin, `supabase/seed-test.sql`, the `.vercel` lint ignore, and this file's corrections. Its preview already went green, so what merging still buys is the **production** fix — confirm production flips to `● Ready` straight afterwards. Note a solo repo can't satisfy GitHub's review requirement (you can't approve your own PR), so the merge itself has to be done by hand.
2. Seed a testable squad: sign in two throwaway addresses via magic link, substitute the four placeholders in `supabase/seed-test.sql` (three emails and a display name), run it, and walk the Home screen by hand. The script aborts rather than committing a half-valid squad, so a clean run is meaningful. This is the first time Part 6 gets exercised against real data, and it must happen before Part 7 builds on top of it.
