...
- **Vercel still needs an account-side diagnosis.** Both the PR preview and `main` deployment failed, but an archive of the exact committed tree passed a clean `npm ci` and `npm run build` with no `.env.local`, and the merged tree is identical to that tested tree. The current Vercel CLI credential is invalid, so its build logs were unavailable here. Re-authenticate the CLI or inspect deployment `dpl_4UfDLbMHrDNkcsKmSdCXa5bFiPh4` in the Vercel dashboard; do not paper over the status by changing otherwise-passing app code. This does not block starting Part 7 locally, but it must be resolved before Part 9 is complete.
